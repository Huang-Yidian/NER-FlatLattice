...
serverUpdateTaskStatus = 'http://127.0.0.1:8181/NERtaskStatus'
...
text = "玉滩水库，位于重庆市大足区境内，2011年下闸蓄水，玉滩水库2015年荣获全国水利工程建设唯一一个鲁班奖。大沟水库，全称重庆璧山区大沟水库水利风景区，位于璧山区西北的云雾山中，始建于1966年，是一座农业灌溉、城镇供水为一体的综合水利枢纽工程。金家坝水库，位于重庆市酉阳县官清乡、小河镇境内，2006年开工建设，2011年并网发电，工程任务以发电为主，兼有旅游、水产养殖等综合效益。钟灵水库，位于重庆市秀山县钟灵镇境内，1982年建成投产，库区两岸风光秀丽，是秀山最大的淡水养鱼基地。龙水湖水库，位于重庆市大足区，修建于50年代末，现已发展成为龙水湖风景区，因其座落于西山脚下，被誉为“重庆西湖”。观音洞水库，位于重庆市渝北区木耳镇，2006年开工建设，水库功能以重庆主城区备用水源及应急水源为主，兼有农业灌溉、农村人畜饮水等综合效益。"
...
async def retrieve(request):
    ...
    try :
        ...
        if len(text) > 0 :
            text = "".join([Q2B(i) for i in text])
            text_set = text.strip().split("。").remove("")
            text_set = [string for string in text_set if len(string)>1]
            ...
            j['taskid'] = taskId;
            requests.post(serverSubmitDataset, headers = headers, data = json.dumps(j));
        else:
            print(len(text))
